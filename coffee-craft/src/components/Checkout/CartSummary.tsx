...
const CartSummary = () => {
  ...
  // 👉 Tính toán giảm giá
  const isVoucherValid = selectedVoucher && subtotal >= (Number(selectedVoucher.minimumOrderValue ?? 0));
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
      ...
      <div className="space-y-4 text-sm text-gray-700 mb-4 max-h-64 overflow-y-auto pr-2">
        {cartItems.map((item) => (
          <div key={`${item.productId}-${item.grindType || "default"}`} className="flex justify-between items-start gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                {item.grindType && <p className="text-xs text-gray-500">Loại xay: {item.grindType}</p>}
                <p className="text-xs text-gray-500">{item.quantity} x {item.price.toLocaleString("vi-VN")}₫</p>
              </div>
            </div>
      ...
      <div className="space-y-4 text-sm text-gray-700">
        ...
        {/* Mã giảm giá */}
        <div>
          <label className="block mb-2 text-sm font-medium text-gray-700">Mã giảm giá</label>
          <div className="flex gap-2">
            ...
            />
            <button onClick={handleApplyVoucher} className="bg-gray-800 text-white px-4 rounded">Áp dụng</button>
            {selectedVoucher && (
              <button onClick={handleClearVoucher} className="text-red-500 text-sm">Xóa</button>
            )}
          </div>
          {error && <p className="text-red-500 text-sm mt-1">{error}</p>}
          {selectedVoucher && (
            <p className="text-green-600 text-sm mt-1">Áp dụng: {selectedVoucher.code}</p>
          )}
          {selectedVoucher && !isVoucherValid && (
            <p className="text-yellow-500 text-sm mt-1">
              Đơn hàng phải đạt tối thiểu {Number(selectedVoucher.minimumOrderValue).toLocaleString("vi-VN")}₫ để áp dụng mã.
            </p>
          )}
        </div>
